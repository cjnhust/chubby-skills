...
- If Codex leaves inline findings on the current head, fix them or resolve the threads. If the head changes after that review, request one fresh current-head review; if the head stays the same, GitHub's native conversation-resolution rule is enough to unblock merge.
...
- If a finding is already fixed on the latest head but the old review thread stays open, resolve that thread manually. Only request one fresh current-head review when the PR head actually changes or the latest review was dismissed.
