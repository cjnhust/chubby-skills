...
## Step 0.5: Resolve Style Authority Before Generation

`EXTEND.md` controls runtime preferences such as provider, model, quality, and size. It is **not** the source of truth for palette, typography, diagram posture, or other art-direction constraints.
Treat those runtime preferences as this skill's direct defaults. If the current run needs shared style or other request-scoped intent, keep that intent explicit in saved prompt artifacts or caller-provided inputs instead of hiding it in `EXTEND.md`.

Preserve the user's chosen default provider/model as the baseline execution path.

- Do **not** switch provider or model by default just because another model might be better for text density, mixed-language labels, or similar heuristics.
- Treat alternative models as optional suggestions, not silent routing decisions.
- Only override the baseline provider/model when one of these is true:
  - the user explicitly asks to use another provider or model
  - the current provider/model cannot satisfy a hard requirement such as reference-image support or the required size/ratio API surface
  - the current attempt already failed and the user agrees to try a fallback
- If you believe another model may help, tell the user briefly and keep the default unless they opt in or a hard requirement forces the change

When this skill is used standalone without an upstream visual orchestrator, decide whether the request includes a reusable visual style:

1. If the user provides an explicit style brief, named aesthetic, or asks for a specific established look:
   - Treat that style as a first-class input.
   - Do **not** bury it inside a one-off workspace prompt.
   - If the requested style matches a shared house style, prefer a saved style-authority file produced by an upstream bridge or a reusable prompt-library entry. Do not implicitly depend on unrelated pipeline notes or higher-level workflow state.
   - If the style should be reused across projects, persist it under `$HOME/.baoyu-skills/baoyu-image-gen/prompt-library/<style-slug>.md`.
   - If the style belongs only to the current document or workspace, keep it in a workspace-local prompt file.
2. Keep reusable style instructions and task-specific narrative in separate files whenever possible.
   - Style authority file: palette, typography, composition rules, prohibited effects, rendering posture
   - Task content file: subject matter, story, modules, labels, constraints unique to this image
   - If the user provided a raw visual source such as a Mermaid block, diagram spec, or detailed prompt text, preserve that source as a canonical saved artifact instead of rewriting it into prose-only task content.
3. Prefer `--promptfiles <style-file> <task-file>` over a single mixed prompt when style fidelity matters.
4. When the user has explicitly specified a style that differs from baoyu's default output tendencies, invoke `baoyu-style-bridge` or manually normalize the style authority before generation.
   - For non-trivial request-level workflows, prefer entering through `baoyu-visual-pipeline` first so style preparation happens before this leaf renderer is invoked.
5. Remove or rewrite any conflicting visual cues in saved prompt files before generating. Do not leave pastel/light-background/cyan cues in place when the requested style is matte-black, high-contrast, and accent-orange.
6. When style fidelity matters, treat the files passed through `--promptfiles` as the executable source of truth. Do not rely on unsaved chat context or re-summarize the style loosely at generation time.
7. Do not silently add semantic exclusions or structural rewrites that the user did not request. If the renderer needs a derived prompt narrative, keep it secondary to the canonical user source.

## Step 0.6: Respect Layer Boundaries

This skill is a rendering capability.

- It consumes explicit prompt files, reference images, and its own `EXTEND.md`.
- It may be called by a higher-level workflow, but it does **not** own workspace creation, shared bundle persistence, review policy, or downstream routing.
- If a higher-level workflow needs a shared house style, it must materialize that style into saved prompt files or a reusable style-authority file before invoking this skill.
- If the current run includes a canonical visual source artifact, do not replace it with an opaque prompt rewrite and treat that rewrite as the only semantic input.
- If the current request has intermediate artifacts, also read [../../owned/shared/references/working-artifact-contract.md](../../owned/shared/references/working-artifact-contract.md). The caller or flow-local orchestrator should provide the working location and explicit file paths from there. This skill may use those files, but it does not prescribe the overall directory layout.
...
After the first successful generation for a user-facing image:

- Inspect the output against the saved style authority and any structure source or reference image that the current task provided.
- Summarize notable deviations in plain language, for example:
  - background tone or contrast drift
  - emphasis color overuse or underuse
  - structure drift, missing nodes, or extra decorative elements
  - text density or readability problems
- Do **not** auto-regenerate or hard-fail solely on style drift unless the user explicitly asked for strict enforcement.
- Let the user decide whether to keep the first pass, regenerate for style, regenerate for structure, or stop.
