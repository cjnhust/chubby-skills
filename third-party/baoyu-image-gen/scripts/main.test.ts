...
test("image format detection and output path resolution follow actual image bytes", () => {
  ...
  assert.equal(detectImageExtension(jpegBytes), ".jpg");
  assert.equal(detectImageExtension(pngBytes), ".png");
  assert.equal(detectImageExtension(webpBytes), ".webp");

  assert.match(resolveOutputImagePath("out/sample", jpegBytes), /out[\\/]+sample\.jpg$/);
  assert.match(resolveOutputImagePath("out/sample.png", jpegBytes), /out[\\/]+sample\.jpg$/);
  assert.match(resolveOutputImagePath("out/sample.jpeg", jpegBytes), /out[\\/]+sample\.jpeg$/);
  assert.match(resolveOutputImagePath("out/sample.png", pngBytes), /out[\\/]+sample\.png$/);
  assert.match(resolveOutputImagePath("out/sample.jpg", webpBytes), /out[\\/]+sample\.webp$/);
});
