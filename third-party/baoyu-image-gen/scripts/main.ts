...
function matchesSignature(imageData: Uint8Array, offset: number, signature: number[]): boolean {
  if (imageData.length < offset + signature.length) return false;
  return signature.every((value, index) => imageData[offset + index] === value);
}

export function detectImageExtension(imageData: Uint8Array): ".png" | ".jpg" | ".webp" | ".gif" | null {
  if (matchesSignature(imageData, 0, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])) return ".png";
  if (matchesSignature(imageData, 0, [0xff, 0xd8, 0xff])) return ".jpg";
  if (
    matchesSignature(imageData, 0, [0x52, 0x49, 0x46, 0x46]) &&
    matchesSignature(imageData, 8, [0x57, 0x45, 0x42, 0x50])
  ) {
    return ".webp";
  }
  if (
    matchesSignature(imageData, 0, [0x47, 0x49, 0x46, 0x38, 0x37, 0x61]) ||
    matchesSignature(imageData, 0, [0x47, 0x49, 0x46, 0x38, 0x39, 0x61])
  ) {
    return ".gif";
  }
  return null;
...
function normalizeComparableExtension(ext: string): string {
  return ext.toLowerCase() === ".jpeg" ? ".jpg" : ext.toLowerCase();
}

export function resolveOutputImagePath(outputPath: string, imageData: Uint8Array): string {
  const normalizedOutputPath = normalizeOutputImagePath(outputPath);
  const inferredExtension = detectImageExtension(imageData);
  if (!inferredExtension) return normalizedOutputPath;

  const currentExtension = path.extname(normalizedOutputPath);
  if (!currentExtension) return `${normalizedOutputPath}${inferredExtension}`;

  if (normalizeComparableExtension(currentExtension) === inferredExtension) {
    return normalizedOutputPath;
  }

  return path.join(
    path.dirname(normalizedOutputPath),
    `${path.basename(normalizedOutputPath, currentExtension)}${inferredExtension}`
  );
}
...
async function writeImage(outputPath: string, imageData: Uint8Array): Promise<string> {
  const resolvedOutputPath = resolveOutputImagePath(outputPath, imageData);
  await mkdir(path.dirname(resolvedOutputPath), { recursive: true });
  await writeFile(resolvedOutputPath, imageData);
  if (resolvedOutputPath !== outputPath) {
    console.error(`Adjusted output extension to match image data: ${resolvedOutputPath}`);
  }
...
async function generatePreparedTask(task: PreparedTask): Promise<TaskResult> {
  ...
  while (attempts < MAX_ATTEMPTS) {
    ...
    try {
      ...
      const outputPath = await writeImage(task.outputPath, imageData);
      return {
        id: task.id,
