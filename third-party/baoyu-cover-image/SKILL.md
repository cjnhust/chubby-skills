...
When EXTEND.md includes a custom palette created by a shared visual profile or `baoyu-style-bridge`, treat that custom palette as the saved default art direction. Do not replace it with a built-in palette suggestion unless the user explicitly asks to change style.
...
`prompts/cover.md` is the source of truth for cover styling. The confirmed style, palette, rendering, and any explicit user style brief must be written into this file before generation.
...
4. **Generate**: Call skill with the saved prompt file, output path, aspect ratio. Do not regenerate style instructions ad hoc from chat context.
5. After the first successful generation, summarize visible deviations in style, composition, reference fidelity, or readability before any optional regeneration. Do not auto-regenerate unless generation failed or the user asks for another pass.
