...
If EXTEND.md defines `preferred_palette`, `preferred_rendering`, or a custom palette in `custom_palettes`, surface that saved preference first in the recommended option instead of falling back to a built-in palette suggestion.
...
multiSelect: false
options:
  - label: "[saved preferred palette or auto-recommended palette] (Recommended)"
    description: "[reason based on saved preference and content signals]"
  - label: "warm"
...
If a saved custom palette is recommended, use its actual name in the option label, for example `engineering-orange-dark (Recommended)`, and summarize its intent briefly.
