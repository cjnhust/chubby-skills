...
preferred_palette: null   # built-in palette name, custom palette name, or null for auto-select
...
| `preferred_palette` | string | null | Built-in palette name, custom palette name, or null for auto |
...
`preferred_palette` may also point at a palette defined in `custom_palettes[].name`.
...
### Example: Use a custom palette as the default
...
custom_palettes:
  - name: engineering-orange-dark
    description: "Dark engineering palette with restrained orange emphasis"
    colors:
    ...
    best_for: "Architecture article, technical deck, system blueprint cover"
---
```
