...
This skill is a deliverable-specific leaf in the visual family. If the request is visual-only and needs family-level routing or art-direction sequencing, prefer `baoyu-visual-pipeline` as the entrypoint; if it belongs to a document/deck storyline, use `engineering-story-pipeline`.
...
Saved artifacts are the style authority for execution:
- `outline.md` carries the canonical `STYLE INSTRUCTIONS`
- slide prompt files carry slide-level style decisions
- image generation must consume those saved artifacts rather than re-inferring style from chat context
...
- **Recommended style**: [saved custom style or preset] (saved preference first, then content signals)
...
options:
  - label: "{saved style or recommended preset} (Recommended)"
    description: "Use saved custom style from EXTEND.md when present; otherwise best match based on content analysis"
...
If EXTEND.md `style` points to a custom style generated by `baoyu-style-bridge` or workspace materialization, present that custom style as the first option instead of collapsing back to a built-in preset such as `blueprint`.
...
6. After the first image pass, summarize deck-level deviations in style consistency, slide readability, and cover/body/end-page differentiation before broad regeneration or merge
7. Prefer targeted prompt fixes and selective regeneration over regenerating the whole deck by reflex
