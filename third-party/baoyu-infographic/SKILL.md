...
If the current task already provides a saved style-authority artifact, a targeted workspace-local config, or an explicit style override from the user, treat that explicit style authority as primary. Do not fall back to a gallery style just because the infographic skill has its own local style catalog.
...
**3.3 Explicit style override wins**: If the user has already supplied a clear visual style brief, or the current task provides a saved style-authority artifact, preserve that style authority and map layout choices around it. Do not silently revert to stock styles such as `craft-handmade` or `technical-schematic`.
...
6. Any explicit style authority from the current task or current user brief

If an explicit style authority exists, patch `prompts/infographic.md` so its palette, background, typography, emphasis rules, and prohibited effects reflect that authority, even when the selected layout comes from the infographic gallery.

`prompts/infographic.md` is the executable style source of truth. Do not rely on unsaved chat-only style instructions once generation starts.
...
3. Call with the saved prompt file and output path. Do not regenerate the style ad hoc during this step.
4. On failure, auto-retry once
5. After the first successful render, summarize any visible deviations in style, structure, or readability before optional regeneration. Do not automatically regenerate unless the user asks or the current workflow explicitly requires another pass.
