...
For repeated maintenance, the local source skills remain the authoring source of truth; the publish repo should be treated as a mirror updated by sync, not as the place where managed skill content is authored directly.
...
0. Resolve local defaults before inventory when available.
   ...
   - For repeated publication work against one stable public repo, treat `default_publish_repo` as the fixed working copy instead of creating a fresh sibling staging directory every time.
   ...
   - When the user wants the repeated local flow to cover branch creation and base refresh as well, prefer `python3 scripts/prepare_incremental_pr.py --skill-root <path> [...]` so the fixed publish repo is refreshed from the base branch, a PR branch is created or reset deliberately, the selected skill roots are synced, the signed publish sync manifest is rewritten, and strict local checks rerun before commit.
   - For a publish repo that should not accept direct skill-bundle edits, install a CI guard that rejects managed skill changes unless they match a signed sync manifest generated by the local sync helper.
...
10. Decide whether post-publish Codex GitHub integration is appropriate.
   ...
   - For a public skills repo that is already sanitized and intentionally shareable:
     ...
     - trigger Codex review exactly once per PR head: use repository auto review or reviewer request when available, and keep `@codex review` as the manual fallback instead of stacking multiple trigger paths on the same head
     ...
     - when the latest head already fixes a Codex finding but GitHub leaves the old thread open, manually resolve that thread and then request one fresh current-head review through the same single trigger path already chosen for the PR
...
- Do not treat the publish repo as a second writable source of truth for managed skill content when the intended model is local-source-first mirroring.
...
- `scripts/prepare_incremental_pr.py`: refresh the fixed publish repo from the base branch, create or reuse a PR branch intentionally, sync selected skill roots, and rerun the local publish checks in one repeated-update flow
- `scripts/publish_sync_manifest.py`: record and sign a manifest for managed skill changes so the publish repo can reject direct edits that did not come from the local sync flow
- `scripts/verify_publish_sync.py`: CI-side verification that managed skill changes on a PR match the recorded signed publish sync manifest
