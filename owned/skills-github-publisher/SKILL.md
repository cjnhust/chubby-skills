...
0. Resolve local defaults before inventory when available.
   ...
   - When the user wants the repeated local flow to cover branch creation and base refresh as well, prefer `python3 scripts/prepare_incremental_pr.py --skill-root <path> [...]` so the fixed publish repo is refreshed from the base branch, a PR branch is created or resumed deliberately, the selected skill roots are synced, the publish sync manifest is rewritten, and strict local checks rerun before commit.
   - For a publish repo that should not accept direct skill-bundle edits, install a CI guard that rejects managed skill changes unless they match a sync manifest generated by the local sync helper.
...
10. Decide whether post-publish Codex GitHub integration is appropriate.
   ...
   - For a public skills repo that is already sanitized and intentionally shareable:
     ...
     - if the repo installs a Codex hard gate, prefer requiring a current-head Codex review plus resolved Codex review threads instead of assuming every Codex review must end in a clean approval state
...
- `scripts/publish_sync_manifest.py`: record a manifest for managed skill changes so the publish repo can reject direct edits that did not come from the local sync flow
- `scripts/verify_publish_sync.py`: CI-side verification that managed skill changes on a PR match the recorded publish sync manifest
