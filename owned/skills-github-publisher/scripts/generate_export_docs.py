...
def build_codex_setup(root: Path) -> str:
    ...
    if review_gate_enabled:
        lines.append("- Keep the `codex-review-gate` workflow green; that gate should only require a current-head Codex review on the latest PR head.")
        ...
        lines.append("- If you are introducing the hard-gate workflows for the first time, keep the bootstrap PR same-repo and trigger one current-head Codex review so the required status gets stamped on that head.")
    else:
    ...
    lines.append("- If Codex leaves inline findings on the current head, fix them or resolve the threads. If the head changes after that review, request one fresh current-head review; if the head stays the same, GitHub's native conversation-resolution rule is enough to unblock merge.")
    ...
    lines.append("- If a finding is already fixed on the latest head but the old review thread stays open, resolve that thread manually. Only request one fresh current-head review when the PR head actually changes or the latest review was dismissed.")
    ...
    lines.append("")
    lines.append("Manual fallback review request:")
    lines.append("")
    lines.append("```text")
